...
export default function DashboardPage() {
  ...
  const [newJobPricingModel, setNewJobPricingModel] = useState<"fixed" | "hourly" | "negotiable">("fixed");
    ...
    try {
      ...
      if (profileRes.ok) {
        ...
        if (data.profile) {
          ...
          setProfHourlyRate(data.profile.hourlyRate ? String(data.profile.hourlyRate) : "");
  ...
  const fetchSession = useCallback(async () => {
    try {
      ...
      const approvedRoles = roles.filter((r: { name: string; status: string }) => r.status === "approved");
      ...
      const hasClient = approvedRoles.some((r: { name: string; status: string }) => r.name === "CLIENT");
      const hasProvider = approvedRoles.some((r: { name: string; status: string }) => r.name === "PROVIDER");
  ...
  const handlePostJobSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newJobTitle || !newJobDescription || !newJobCategoryId || !newJobBudget) {
  ...
  // Filter approved roles
  const approvedRoles = session.roles.filter((r) => r.status === "approved").map((r) => r.name);
  const hasClientRole = approvedRoles.includes("CLIENT");
  const hasProviderRole = approvedRoles.includes("PROVIDER");

  // Filter client's contracts
  const clientContracts = contracts.filter((c) => c.clientId === session.userId);
  // Filter editor's contracts
  const providerContracts = contracts.filter((c) => c.providerId === session.userId);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans">
      ...
      <nav className="border-b border-slate-900 bg-slate-950/80 backdrop-blur-md sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <div className="h-8 w-8 rounded-lg bg-gradient-to-r from-purple-500 to-blue-500 p-1 flex items-center justify-center font-bold text-slate-950 text-sm">
              M
            </div>
          ...
          <div className="flex items-center space-x-6">
            {/* Role switcher toggle (only if they have both roles approved) */}
            {hasClientRole && hasProviderRole && (
              <div className="bg-slate-900 p-1 rounded-lg border border-slate-800 flex items-center space-x-1">
                <button
            ...
            <button
              ...
              className="text-slate-400 hover:text-red-400 transition-colors p-1.5 rounded-lg hover:bg-slate-900 border border-transparent hover:border-slate-800"
              title="Sign Out"
            >
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8">
        ...
        <header className="mb-8">
          ...
          <p className="text-slate-400 mt-1">
            ...
            <span className={`font-bold uppercase ${activeView === "CLIENT" ? "text-purple-400" : "text-blue-400"}`}>
              {activeView === "CLIENT" ? "Client / Buyer" : "Service Provider / Editor"}
            </span>
          </p>
        </header>
          ...
          <section className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              <Card
                ...
                className="border-slate-900 bg-slate-950 hover:border-purple-500/30 transition-all cursor-pointer group"
              >
                <CardHeader>
                  <div className="h-10 w-10 rounded-lg bg-purple-500/10 text-purple-400 flex items-center justify-center mb-2">
                    <Plus className="h-5 w-5" />
                  </div>
                  <CardTitle className="text-lg group-hover:text-purple-400 transition-colors">Post a Service Request</CardTitle>
                  <CardDescription className="text-slate-400">Describe your project, upload requirements, and set budget.</CardDescription>
                </CardHeader>
              </Card>
              ...
              >
                <CardHeader>
                  ...
                  <CardTitle className="text-lg group-hover:text-purple-400 transition-colors">Search Service Providers</CardTitle>
                  <CardDescription className="text-slate-400">Browse experts by category, skills, and portfolio reviews.</CardDescription>
                </CardHeader>
              </Card>
              ...
              >
                <CardHeader>
                  ...
                  <CardTitle className="text-lg group-hover:text-purple-400 transition-colors">Manage Escrow Milestones</CardTitle>
                  <CardDescription className="text-slate-400">Verify milestones, release funded payments, and view contracts.</CardDescription>
                </CardHeader>
              </Card>
              ...
              >
                <CardHeader>
                  ...
                  <CardTitle className="text-lg group-hover:text-purple-400 transition-colors">Edit Client Profile</CardTitle>
                  <CardDescription className="text-slate-400">Modify your display name, contact phone, and address.</CardDescription>
                </CardHeader>
              </Card>
            </div>
              ...
              <div className="lg:col-span-2 space-y-4">
                ...
                {postedJobs.length === 0 ? (
                  <div className="rounded-xl border border-slate-900 bg-slate-900/10 p-12 text-center text-slate-500">
                    <p className="font-semibold text-slate-400">No active job posts yet</p>
                    <p className="text-xs mt-1">Get started by creating your first service request project!</p>
                    <Button onClick={handleOpenPostJob} className="mt-4 bg-purple-600 hover:bg-purple-500 text-slate-950 font-semibold text-xs">
                      Create Request
                    </Button>
                  </div>
                ) : (
                  <div className="space-y-4">
                    {postedJobs.map((job) => (
                      <Card key={job.id} className="border-slate-900 bg-slate-950 p-5 hover:border-slate-800 transition-all">
                        <div className="flex justify-between items-start">
                          <div>
                            <span className="px-2 py-0.5 rounded bg-purple-500/10 text-purple-400 text-xs font-semibold">
                              {job.category.name}
                            </span>
                            <h4 className="text-lg font-bold text-slate-100 mt-2">{job.title}</h4>
                            <p className="text-sm text-slate-400 mt-1 line-clamp-2">{job.description}</p>
                          </div>
                          <div className="text-right">
                            <span className="text-lg font-bold text-purple-400">${Number(job.budget).toFixed(2)}</span>
                            <p className="text-xs text-slate-500 capitalize">{job.pricingModel} model</p>
                          </div>
                        </div>

                        <div className="flex flex-wrap items-center justify-between mt-4 pt-4 border-t border-slate-900 text-xs text-slate-400">
                          <div className="flex space-x-4">
                            ...
                            <span className={`capitalize font-semibold ${job.status === "posted" ? "text-yellow-400" : "text-green-400"}`}>
                              Status: {job.status}
                            </span>
                          </div>
                          ...
                          <Button
                            ...
                            className="bg-purple-600 hover:bg-purple-500 text-slate-950 font-bold px-3 py-1.5 h-auto text-xs"
                          >
                  ...
                  <CardContent className="space-y-4 text-xs text-slate-400">
                    <div className="flex justify-between border-b border-slate-900 pb-2">
                      ...
                      <span className="text-slate-200 font-bold">
                        ${clientContracts.reduce((sum, c) => sum + Number(c.agreedPrice), 0).toFixed(2)}
                      </span>
                    </div>
                    <div className="flex justify-between border-b border-slate-900 pb-2">
                      <span>Active Assignments:</span>
                      <span className="text-slate-200">{clientContracts.filter(c => c.status === 'active').length}</span>
                    </div>
                    <div className="flex justify-between pb-2">
                      <span>Total Contracts:</span>
                      <span className="text-slate-200">{clientContracts.length}</span>
                    </div>
                  </CardContent>
                </Card>
              </div>
          ...
          <section className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <Card
                ...
                className="border-slate-900 bg-slate-950 hover:border-blue-500/30 transition-all cursor-pointer group"
              >
                <CardHeader>
                  <div className="h-10 w-10 rounded-lg bg-blue-500/10 text-blue-400 flex items-center justify-center mb-2">
                    <Search className="h-5 w-5" />
                  </div>
                  <CardTitle className="text-lg group-hover:text-blue-400 transition-colors">Browse Available Jobs</CardTitle>
                  <CardDescription className="text-slate-400">Find new clients seeking editing, design, or consulting work.</CardDescription>
                </CardHeader>
              </Card>
              ...
              >
                <CardHeader>
                  ...
                  <CardTitle className="text-lg group-hover:text-blue-400 transition-colors">My Proposals</CardTitle>
                  <CardDescription className="text-slate-400">View and track status updates on your submitted bids and pitches.</CardDescription>
                </CardHeader>
              </Card>
              ...
              >
                <CardHeader>
                  ...
                  <CardTitle className="text-lg group-hover:text-blue-400 transition-colors">Edit Expert Profile</CardTitle>
                  <CardDescription className="text-slate-400">Modify your bio, list expert skills, and showcase your portfolio.</CardDescription>
                </CardHeader>
              </Card>
            </div>
              ...
              <div className="lg:col-span-2 space-y-4">
                ...
                {providerContracts.length === 0 ? (
                  <div className="rounded-xl border border-slate-900 bg-slate-900/10 p-12 text-center text-slate-500">
                    ...
                    <p className="text-xs mt-1">Start bidding on job openings to secure your first project assignment!</p>
                    <Button onClick={() => setIsBrowseJobsOpen(true)} className="mt-4 bg-blue-600 hover:bg-blue-500 text-slate-950 font-semibold text-xs">
                      Browse Jobs
                    </Button>
                  </div>
                ) : (
                  <div className="space-y-4">
                    {providerContracts.map((c) => (
                      <Card key={c.id} className="border-slate-900 bg-slate-950 p-5 hover:border-slate-800 transition-all">
                        <div className="flex justify-between items-start">
                          <div>
                            <span className="px-2 py-0.5 rounded bg-blue-500/10 text-blue-400 text-xs font-semibold">
                              Contract Assigned
                            </span>
                            <h4 className="text-lg font-bold text-slate-100 mt-2">{c.jobTitle}</h4>
                            <p className="text-sm text-slate-400 mt-1">{c.jobDescription}</p>
                            <p className="text-xs text-slate-500 mt-2">Client: {c.clientName}</p>
                          </div>
                          <div className="text-right">
                            <span className="text-lg font-bold text-blue-400">${Number(c.agreedPrice).toFixed(2)}</span>
                ...
                <Card className="border-slate-900 bg-slate-950">
                  ...
                  <CardContent className="space-y-4 text-xs text-slate-400">
                    <div className="flex justify-between border-b border-slate-900 pb-2">
                      ...
                      <span className="text-slate-200 font-bold">
                        ${providerContracts.filter(c => c.status === 'completed').reduce((sum, c) => sum + Number(c.agreedPrice), 0).toFixed(2)}
                      </span>
                    </div>
                    <div className="flex justify-between border-b border-slate-900 pb-2">
                      <span>Average Rating:</span>
                      <span className="text-slate-200">{providerProfile?.averageRating ? `${Number(providerProfile.averageRating).toFixed(2)} ★` : '0.00 ★'}</span>
                    </div>
                    <div className="flex justify-between pb-2">
                      <span>Submitted Bids:</span>
                      <span className="text-slate-200">{myProposals.length}</span>
                    </div>
                  </CardContent>
                </Card>
              </div>
      ...
      <Dialog open={isPostJobOpen} onOpenChange={setIsPostJobOpen}>
        <DialogContent className="bg-slate-950 border border-slate-800 text-slate-100 max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
          ...
          <form onSubmit={handlePostJobSubmit} className="space-y-4 my-2">
            ...
            <div className="space-y-1">
              ...
              <textarea
                ...
                required
                rows={4}
                className="bg-slate-900 border border-slate-800 text-slate-100 rounded-md p-2 w-full focus:outline-none focus:ring-1 focus:ring-purple-500 text-sm"
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1">
                ...
                <select
                  id="category"
                  required
                  className="bg-slate-900 border border-slate-800 text-slate-100 rounded-md p-2 w-full text-sm focus:outline-none focus:ring-1 focus:ring-purple-500"
                  value={newJobCategoryId}
                  onChange={(e) => setNewJobCategoryId(e.target.value)}
                >
                  <option value="">Select subcategory</option>
                  {categories.map((cat) => (
                    <optgroup key={cat.id} label={cat.name} className="text-slate-400 bg-slate-950">
                      {(cat.subcategories || []).map((sub) => (
                        <option key={sub.id} value={sub.id} className="text-slate-100">
                          {sub.name}
                        </option>
                      ))}
                    </optgroup>
              ...
              <div className="space-y-1">
                ...
                <select
                  ...
                  onChange={(e) => setNewJobPricingModel(e.target.value as "fixed" | "hourly" | "negotiable")}
                >
            ...
            <div className="space-y-1">
              <Label htmlFor="location">Location (optional for local services)</Label>
              <Input
                id="location"
                className="bg-slate-900 border-slate-800 text-slate-100"
            ...
            <DialogFooter className="pt-4">
              ...
              <Button type="submit" className="bg-purple-600 hover:bg-purple-500 text-slate-950 font-bold">
                Submit Request
              </Button>
            </DialogFooter>
          </form>
      ...
      <Dialog open={isViewBidsOpen} onOpenChange={setIsViewBidsOpen}>
        <DialogContent className="bg-slate-950 border border-slate-800 text-slate-100 max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Bids for: {selectedJob?.title}</DialogTitle>
            <DialogDescription className="text-slate-400">
              Review proposals submitted by expert editors and assign this contract.
            </DialogDescription>
          </DialogHeader>
          ...
          {jobProposals.length === 0 ? (
            ...
            <div className="space-y-4 my-2">
              {jobProposals.map((prop) => (
                <Card key={prop.id} className="border-slate-900 bg-slate-900/30 p-4 space-y-3">
                  <div className="flex justify-between items-start">
                    <div>
                      <h4 className="font-bold text-slate-200 text-md flex items-center space-x-1">
                    ...
                    <div className="text-right">
                      <p className="text-md font-extrabold text-purple-400">${Number(prop.bidAmount).toFixed(2)}</p>
                      <p className="text-[11px] text-slate-500">
                        {prop.estimatedDays ? `Est. delivery: ${prop.estimatedDays} days` : "Negotiable timeline"}
                      </p>
                    </div>
                  </div>

                  <p className="text-xs text-slate-300 italic bg-slate-950/40 p-2.5 rounded border border-slate-950">
                    &quot;{prop.proposalText}&quot;
                  </p>

                  <div className="flex flex-wrap gap-1">
                    {(prop.providerSkills || []).map((skill) => (
                      <span key={skill} className="px-1.5 py-0.5 rounded bg-slate-900 text-slate-400 text-[10px]">
                        {skill}
                      </span>
                    ))}
                  </div>

                  {prop.status === "pending" && selectedJob?.status === "posted" ? (
                    <div className="flex justify-end space-x-2 pt-2 border-t border-slate-900">
                      ...
                      <Button
                        ...
                        className="bg-green-600 hover:bg-green-500 text-slate-950 font-bold px-3 py-1 text-xs"
                      >
                        Accept & Assign
                      </Button>
                    </div>
                    ...
                    <div className="flex justify-end pt-2 border-t border-slate-900">
                      <span className={`text-xs font-bold capitalize ${
                        prop.status === "accepted" ? "text-green-400" : "text-slate-500"
                      }`}>
      ...
      <Dialog open={isSearchProvidersOpen} onOpenChange={setIsSearchProvidersOpen}>
        <DialogContent className="bg-slate-950 border border-slate-800 text-slate-100 max-w-xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
          ...
          {providers.length === 0 ? (
            ...
            <div className="space-y-4 my-2">
              {providers.map((p) => (
                <Card key={p.id} className="border-slate-900 bg-slate-900/30 p-4 space-y-2">
                  <div className="flex justify-between items-start">
                    <div>
                      <h4 className="font-bold text-slate-200 text-md">{p.fullName}</h4>
                      <p className="text-xs text-slate-400 mt-1">{p.bio || "No biography provided"}</p>
                    </div>
                    <div className="text-right">
                      {p.hourlyRate ? (
                        <p className="text-sm font-bold text-purple-400">${Number(p.hourlyRate).toFixed(2)}/hr</p>
                      ) : (
                        <p className="text-xs text-slate-500 font-medium">Negotiable</p>
                      )}
                      <p className="text-[11px] text-yellow-400 mt-0.5">{Number(p.averageRating).toFixed(2)} ★</p>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-1 pt-2 border-t border-slate-900">
                    {p.skills.length > 0 ? (
                      p.skills.map((s) => (
                        <span key={s} className="px-1.5 py-0.5 rounded bg-slate-900 text-slate-400 text-[10px]">
                          {s}
                        </span>
                      ))
                    ) : (
                      <span className="text-[10px] text-slate-600">No skills listed</span>
                    )}
                  </div>
                </Card>
              ))}
          ...
          {clientContracts.length === 0 ? (
            ...
            <div className="space-y-4 my-2">
              {clientContracts.map((c) => (
                <Card key={c.id} className="border-slate-900 bg-slate-900/30 p-4 space-y-2">
                  <div className="flex justify-between items-start">
                    <div>
                      <h4 className="font-bold text-slate-200 text-sm">{c.jobTitle}</h4>
                      <p className="text-xs text-slate-400 mt-0.5">Assigned Editor: {c.providerName}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-sm font-bold text-purple-400">${Number(c.agreedPrice).toFixed(2)}</p>
                      <p className="text-[10px] text-slate-500">Started: {new Date(c.startedAt).toLocaleDateString()}</p>
                    </div>
                  </div>
                  <div className="pt-2 border-t border-slate-900 flex justify-between items-center text-xs">
                    <span className="text-slate-400">Status:</span>
                    <span className="text-green-400 font-bold capitalize">{c.status}</span>
                  </div>
                </Card>
              ))}
            </div>
          ...
          {availableJobs.length === 0 ? (
            ...
            <div className="space-y-4 my-2">
              {availableJobs.map((job) => (
                <Card key={job.id} className="border-slate-900 bg-slate-900/30 p-4 space-y-3">
                  <div className="flex justify-between items-start">
                    <div>
                    ...
                    <div className="text-right">
                      <p className="text-sm font-bold text-blue-400">${Number(job.budget).toFixed(2)}</p>
                      <p className="text-[10px] text-slate-500 capitalize">{job.pricingModel}</p>
                    </div>
                  </div>

                  <div className="pt-2 border-t border-slate-950 flex justify-between items-center text-xs text-slate-500">
                    <span>Deadline: {job.deadline ? new Date(job.deadline).toLocaleDateString() : "No deadline"}</span>
                    
                    {job.hasBid ? (
                      <span className="text-slate-600 font-bold">Proposal Submitted</span>
                    ) : (
                      <Button
                        onClick={() => {
                          setSelectedJob(job);
                        ...
                        className="bg-blue-600 hover:bg-blue-500 text-slate-950 font-bold px-3 py-1 h-auto text-xs"
                      >
                        Submit Bid
                      </Button>
                    )}
          ...
          <form onSubmit={handleBidSubmit} className="space-y-4 my-2">
            ...
            <div className="space-y-1">
              ...
              <textarea
                ...
                required
                rows={4}
                className="bg-slate-900 border border-slate-800 text-slate-100 rounded-md p-2 w-full focus:outline-none focus:ring-1 focus:ring-blue-500 text-sm"
            ...
            <DialogFooter>
              ...
              <Button type="submit" className="bg-blue-600 hover:bg-blue-500 text-slate-950 font-bold">
                Submit Bid
              </Button>
            </DialogFooter>
          </form>
          ...
          {myProposals.length === 0 ? (
            ...
            <div className="space-y-4 my-2">
              {myProposals.map((prop) => (
                <Card key={prop.id} className="border-slate-900 bg-slate-900/30 p-4 space-y-2">
                  <div className="flex justify-between items-start">
                    <div>
                      <h4 className="font-bold text-slate-200 text-sm">{prop.jobTitle}</h4>
                      <p className="text-[11px] text-slate-400 mt-1">Client: {prop.clientName}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-sm font-bold text-blue-400">${Number(prop.bidAmount).toFixed(2)}</p>
                      <p className="text-[10px] text-slate-500">
                        {prop.estimatedDays ? `${prop.estimatedDays} days delivery` : "Negotiated time"}
                      </p>
                    </div>
                  </div>
                  <div className="pt-2 border-t border-slate-900 flex justify-between items-center text-xs">
                    <span className="text-slate-500">Your pitch: &quot;{prop.proposalText}&quot;</span>
      ...
      <Dialog open={isEditProfileOpen} onOpenChange={setIsEditProfileOpen}>
        <DialogContent className="bg-slate-950 border border-slate-800 text-slate-100 max-w-lg">
          <DialogHeader>
          ...
          <form onSubmit={handleUpdateProfile} className="space-y-4 my-2">
            ...
            <div className="space-y-1">
              <Label htmlFor="profSkills">Expert Skills (comma separated) *</Label>
              <Input
                id="profSkills"
                required
                className="bg-slate-900 border-slate-800 text-slate-100"
              ...
              />
              <p className="text-[10px] text-slate-500">Separate skills with commas</p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="flex items-center justify-between border border-slate-900 bg-slate-900/30 rounded-xl p-3">
                <div>
                  <Label className="text-slate-300 text-xs font-semibold">Available for Work</Label>
                  <p className="text-[9px] text-slate-500">Toggle public listing status</p>
                </div>
                <input
                  type="checkbox"
      ...
      {/* CLIENT: EDIT PROFILE */}
      <Dialog open={isEditClientProfileOpen} onOpenChange={setIsEditClientProfileOpen}>
        <DialogContent className="bg-slate-950 border border-slate-800 text-slate-100 max-w-lg">
          <DialogHeader>
          ...
          <form onSubmit={handleUpdateClientProfile} className="space-y-4 my-2">
            <div className="space-y-1">
