  ...
  try {
    ...
    if (jobId) {
      ...
      const formattedProposals = proposals.map((prop) => ({
        ...
        providerName: prop.provider.providerProfile?.fullName || prop.provider.email,
      ...
      const formattedProposals = proposals.map((prop) => ({
        ...
        clientName: prop.job.client.clientProfile?.fullName || prop.job.client.email,
      }));
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (job.status !== 'posted') {
      return NextResponse.json({ error: 'Job is no longer open for bids' }, { status: 400 });
    }
